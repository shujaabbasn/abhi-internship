...
function AdminPage({goToChat}) {
  ...
  const [fpFieldName,setFpFieldName]=useState('')
  const [fpDescription,setFpDescription]=useState('')
  const [fpEnglish,setFpEnglish]=useState('')
  const [fpUrdu,setFpUrdu]=useState('')
  ...
  async function loadFieldPrompts() {
    const response=await fetch(API+'/field_prompts')
    const data=await response.json()
    setFieldPrompts(data)
  }
  ...
  async function addFieldPrompt() {
    if(!fpFieldName.trim()||!fpDescription.trim()||!fpEnglish.trim()||!fpUrdu.trim()) {
    ...
    const response=await fetch(API+'/field_prompts',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({field_name:fpFieldName,description:fpDescription,english:fpEnglish,urdu:fpUrdu})
    })
    const data=await response.json()
  ...
  async function removeFieldPrompt(fieldName) {
    const response=await fetch(API+'/field_prompts/'+fieldName,{method:'DELETE'})
    const data=await response.json()
    if(!response.ok) {
      toast.error(data.detail||'Error')
      return
  ...
  return (
    <div style={{width:'800px',margin:'0 auto',padding:'20px',boxSizing:'border-box'}}>
      ...
      <h1 style={{margin:'32px 0 0 0',fontSize:'32px'}}>Manage Field Prompts</h1>
      <p style={{fontSize:'13px',color:'#ffffff'}}>
        The description is what gets sent to the LLM to generate the question shown to the user. English and Urdu are only used as a fallback if generation fails.
      </p>
      <div style={{display:'flex',flexDirection:'column',gap:'8px',marginTop:'12px'}}>
        <textarea
          style={{padding:'10px',borderRadius:'8px',border:'1px solid #ffffff',fontSize:'15px',resize:'none'}}
          ...
          value={fpFieldName}
          onChange={(e)=>setFpFieldName(e.target.value)}
        />
        <textarea
          style={{padding:'10px',borderRadius:'8px',border:'1px solid #ffffff',fontSize:'15px',resize:'none'}}
          placeholder="Description (what this field means, given to the LLM)"
          value={fpDescription}
          onChange={(e)=>setFpDescription(e.target.value)}
        />
        <textarea
          style={{padding:'10px',borderRadius:'8px',border:'1px solid #ffffff',fontSize:'15px',resize:'none'}}
          placeholder="English fallback text"
          value={fpEnglish}
          onChange={(e)=>setFpEnglish(e.target.value)}
        />
        <textarea
          style={{padding:'10px',borderRadius:'8px',border:'1px solid #ffffff',fontSize:'15px',resize:'none'}}
          placeholder="Urdu fallback text"
          value={fpUrdu}
          onChange={(e)=>setFpUrdu(e.target.value)}
        />
        <button
          style={{padding:'10px 20px',borderRadius:'8px',backgroundColor:'#179eff',color:'white',border:'none',cursor:'pointer',fontSize:'15px'}}
          onClick={addFieldPrompt}>
          Save
        </button>
      </div>
      ...
      <div style={{marginTop:'24px',display:'flex',flexDirection:'column',gap:'28px'}}>
        {fieldPrompts.map(fp=>(
          <div key={fp.field_name} style={{display:'flex',justifyContent:'space-between',alignItems:'center',paddingBottom:'10px'}}>
            <div style={{color:'#ffffff'}}>
              <b>{fp.field_name}</b>-{fp.description}
              <div style={{fontSize:'13px',color:'#ffffff'}}>
                en: "{fp.english}"
              </div>
              <div style={{fontSize:'13px',color:'#ffffff'}}>
            ...
            <button
              style={{padding:'6px 12px',borderRadius:'6px',backgroundColor:'#ffffff',color:'black',border:'none',cursor:'pointer',fontSize:'13px'}}
              onClick={()=>removeFieldPrompt(fp.field_name)}>
              Delete
            </button>
          </div>
        ))}
