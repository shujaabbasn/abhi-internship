...
def detect_language(query):
    prompt=(
        "Is this message in English or Urdu (Urdu script or Roman Urdu)? "
        "Ignore these banking/project terms when judging the language, since they are commonly "
        "used in English even inside Urdu sentences: "+", ".join(DOMAIN_WORDS_EN)+". "
        "Judge the language only from the grammar and other words. "
        "Reply with only one word: en or ur. Message: "+query
    )
    response=requests.post(OLLAMA_URL,json={
        "model":"llama3",
        "messages":[{"role":"user","content":prompt}],
...
ARABIC_SCRIPT_RANGES=[(0x0600,0x06FF),(0x0750,0x077F),(0xFB50,0xFDFF),(0xFE70,0xFEFF)]

def is_arabic_script_char(char):
    code=ord(char)
...
def contains_unexpected_script_for_urdu(text):
    for char in text:
        if char.isspace() or char.isdigit():
            continue
        if is_arabic_script_char(char):
            continue
        if char in "،۔؟!()-'\".":
            continue
        return True
...
def generate_prompt_message(field_name,language,max_attempts=2):
    ...
    if language=="ur":
        language_instruction=(
            "Write it in Urdu script only, not Roman Urdu, not English. "
            "Address the user directly and respectfully as 'آپ' (you), the way you would speak to them face to face. "
            "Never refer to the user in the third person like 'کسی کا' or 'کسی کے' (someone's). "
            "Use 'نمبر' for 'number' - never 'نومبر', which means the month November, not a number. "
            "Example - for 'the user's account number', the correct output is exactly: "
            "آپ کا اکاؤنٹ نمبر کیا ہے؟"
        )
    else:
        language_instruction="Write it in English, addressing the user directly as 'you'."
    prompt=(
    ...
    for attempt in range(max_attempts):
        ...
        if not result:
            continue
        if language=="ur" and contains_unexpected_script_for_urdu(result):
            continue
        if language=="ur" and "آپ" not in result:
            continue
        if language=="ur" and "نومبر" in result:
            continue
        return result
