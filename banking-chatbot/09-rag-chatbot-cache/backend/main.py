...
def refresh_cache_on_startup():
    refresh_script=os.path.join(os.path.dirname(__file__),"refresh_intents_cache.py")
    subprocess.run(["python3",refresh_script])
...
def chat(request:ChatRequest):
    ...
    for field in required:
        ...
        if field=="wants_info":
            ...
            value=None
        if field in ["account_number","from_account","to_account","recipient_account"] and value and not value.isdigit():
            fields.pop(field,None)
            value=None
        if not value:
